...
class Categories_Lang_Api_Object extends Api_Object_Core {
	...
	public function perform_task()
	{
		...
		switch($this->by)
		{
			case "catid" :
				...
				{
					$this->response_data = $this->_get_categories_lang_by_id($this->check_id_value($this->request['id']));
				}

				break;

			default :
				$this->response_data = $this->get_categories_lang_by_all();
		}
	}
	 ...
	 */
	private function _get_categories_lang_by_id($id)
	{
		// Find incidents
		$this->query = "SELECT c.*";
		$this->query .= "FROM `" . $this->table_prefix . "category_lang` c ";
		$this->query .= "WHER c.category_id = :id ";	
		$this->query .= "ORDER BY category_id ASC";
		...
		foreach ($items as $item)
		{
			...
			else
			{
				...
				$json_categories['category_lang' . $i] = array("category_lang" => $item);
				$this->replar[] = 'category_lang' . $i;
			}
	 ...
	 * @return string
	 */
	public function get_categories_lang_by_all()
	{
		$items = array();
		...
		//find incidents
		$this->query = "SELECT c.id, c.category_id, c.category_title as title, c.category_description as description, 
                c.locale
                FROM `" . $this->table_prefix . "category_lang` c";
		...
		foreach ($items as $item)
		{
			...
			else
			{
				...
				$json_categories['categories_lang' . $i] = array("category_lang" => $item);
				$this->replar[] = 'categories_lang' . $i;
			}
		...
		//create the json array
		$data = array("payload" => array("domain" => $this->domain, "categories_lang" => $json_categories), "error" => $this->api_service->get_error_msg(0));
